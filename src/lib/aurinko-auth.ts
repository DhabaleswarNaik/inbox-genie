...
export const getAurinkoAuthorizationUrl = async (serviceType: 'Google' | 'Office365') => {
    ...
    const user = await db.user.upsert({
        ...
        create: {
            id: userId,
            role: 'user',
            emailAddress: '' // If you want, populate from Clerk here
        },
        update: {}
    })

    console.log("✅ Clerk user exists in DB with role:", user.role)
    ...
    const params = new URLSearchParams({
        ...
        serviceType,
        scopes: 'Mail.Read Mail.ReadWrite Mail.Send Mail.Drafts Mail.All offline_access',
        responseType: 'code',
